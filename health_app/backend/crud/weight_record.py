...
def create_weight_record(
    db: Session, weight_record: schemas.WeightRecordCreate
) -> models.WeightRecord:
    """体重記録を作成する
...
def get_weight_records_by_date(
    db: Session, user_id: int, date: Optional[date] = None
) -> List[models.WeightRecord]:
    ...
    if date:
        query = query.filter(models.WeightRecord.date == date)
...
def get_weight_record_by_id(db: Session, record_id: int) -> models.WeightRecord:
    ...
    """
    return (
        db.query(models.WeightRecord)
        .filter(models.WeightRecord.id == record_id)
        .first()
    )


def update_weight_record(
    db: Session, record_id: int, weight_record: schemas.WeightRecordUpdate
) -> models.WeightRecord:
    """体重記録を更新する
